...
// Тесты для функции parse_flag
void test_parse_flag() {
    printf("Testing parse_flag...\n");
    
    // Тест 1: Корректные флаги в нижнем регистре
    ...
    // Тест 2: Корректные флаги в верхнем регистре
    assert(parse_flag('H') == FLAG_H);
    assert(parse_flag('P') == FLAG_P);
    assert(parse_flag('S') == FLAG_S);
    assert(parse_flag('E') == FLAG_E);
...
void test_factorial() {
    ...
    // Тест 2: Большие значения (в пределах int)
...
// Тесты для функции process_h (кратные числа)
void test_process_h() {
    printf("Testing process_h (will show output)...\n");
    
    // Эти тесты в основном проверяют что функция не падает
...
// Тесты для функции process_p (проверка простого числа)
void test_process_p() {
...
// Тесты для функции process_s (шестнадцатеричное представление)
void test_process_s() {
...
// Тесты для функции process_e (таблица степеней)
void test_process_e() {
...
// Тесты для функции process_a (сумма чисел)
void test_process_a() {
...
// Тесты для функции process_f (факториал)
void test_process_f() {
...
void test_edge_cases() {
    ...
    // Проверяем что функции не падают на граничных значениях
    printf("  Testing with boundary values...\n");
    
    // process_h с различными значениями
    process_h(1);
    process_h(100);
    process_h(101);
    
    // process_p с граничными значениями
    process_p(2);  // наименьшее простое
    process_p(1);  // особый случай
...
void test_integration() {
    ...
    // Создаем тестовые входные данные для каждого флага
