...
int* find_primes(int max_count) {
    ...
    // Определяем верхнюю границу для решета Эратосфена
    int limit;
    if (max_count < 100) {
        limit = 1000;  // достаточно для 100 простых чисел
    } else if (max_count < 1000) {
        limit = 10000; // достаточно для 1000 простых чисел
    } else {
        limit = max_count * 20; // эмпирическая формула
    }
